                         ...
                         vector<int>& pair_i,vector<int>& pair_j,vector<long double> & w_kernel, vector<vector<long double> >& dwdx,vector<int> i_type)
{
    ...
    continuity_den(rho,v,avg_v,drhodt,pair_i,pair_j,dwdx,i_type);
    for(i=0; i<n_fluid_total; i++)
    {
        rho_hts[i]=rho[i]+(drhodt[i]*dt*0.5);
    }
...
//printdata.SaveData<long double, long double, long double, long double,int > (x_hts,v_hts,p,rho_hts,i_type, "half_time");
    ...
    continuity_den(rho_hts,v_hts,avg_v,drhodt,pair_i,pair_j,dwdx,i_type);
    for(i=0; i<n_fluid_total; i++)
    {
        for(l=0; l<dim; l++)
        {
        ...
        rhostar[i]=rho[i]+( drhodt[i]*dt*0.5);
        rho[i]=(2.0*rhostar[i])-rho[i];
    }
    ...
    //pressure_sensor( x,nc, p,headlist, taillist, i_type);
